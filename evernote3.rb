...
total_product = 1
array.each { |i| total_product *= i unless i == 0 }
case array.count(0)
when 0
  result_array = array.map { |i| total_product / i }
when 1
  result_array = Array.new(size, 0)
  result_array[array.index(0)] = total_product
else
  result_array = Array.new(size, 0)
end
result_array.each { |i| puts i }
